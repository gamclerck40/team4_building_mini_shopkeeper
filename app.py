...
tab_table, tab_search, tab_outline,tab_graph = st.tabs(["📅표로 보기","👛가계부 검색","🗒️요약 통계","💸지출 통계"]) 
...
with tab_search:
    ...
    if not filtered_df_s.empty:
        filtered_df_s = ut.set_duration(filtered_df_s, start, end)
...
with tab_table:
    ...
    if not load_data.empty:
        if not filtered_df_s.empty:
            st.header("거래 내역")
            st.subheader(f"[{keyword}] 내용의 거래내역")
...
with tab_outline:
    ...
    if not load_data.empty:
        ...
        all_summary_container = st.container()

        if all_summary_btn:
            with all_summary_container:
                st.subheader("전체 요약 통계")

                col1,col2,col3 = st.columns(3)
                col1.metric("총 수입", f"{summary_all['income']:,} 원")
                col2.metric("총 지출", f"{summary_all['expense']:,} 원")
                col3.metric("잔액", f"{summary_all['balance']:,} 원")
...
with tab_graph:
    ...
    if not load_data.empty:
        ...
        # 기간 필터 지정.
        filtered_df_g = ut.set_duration(load_data, start, end)
